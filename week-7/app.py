from flask import Flask, request, render_template, redirect, session, json, jsonify
...
@app.route("/api/members")  # 取得並轉換資料為json格式
def getMember():
...
@app.route("/api/member", methods=["POST"])  # 修改會員姓名
def changeName():
    changeName = "UPDATE members SET name=%s WHERE username COLLATE utf8mb4_0900_as_cs =%s;"
    username = session["username"]
    print(username)
    getjson = request.get_json()  # 取得json字串 這是字典?
    print(getjson)
    print(type(getjson))
    newName = (getjson['name'], username[0])  # 從前端取得姓名
    print(newName)
    mycursor.execute(changeName, newName)
    mydb.commit()

    mycursor.execute(
        "SELECT name FROM members WHERE username COLLATE utf8mb4_0900_as_cs =%s", username)
    name = mycursor.fetchall()
    print(name)

    checkName = name[0][0] == newName[0]
    checkNew = {}

    if checkName == True:
        checkNew["ok"] = True
        return jsonify(checkNew)
    else:
        checkNew["error"] = True
        return jsonify(checkNew)
